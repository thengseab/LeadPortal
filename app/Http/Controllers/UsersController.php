...
class UsersController extends Controller
{
    ...
    public function dashboard(){
        ...
        $todays=date("Y-m-d");
        $today=room_booking::where('date',$todays)->where('status',0)->get();
        $pedding=room_booking::where('date','>',$todays)->where('status',0)->get();
        $active=room_booking::where('date','<',$todays)->where('status',0)->get();

        return view("admin.index",compact("username","pedding","active","today"));
    }

    public function listsBooking(Request $request,$action){
        $username=Auth::user()->name;
        $todays=date("Y-m-d");
        ...
        return view("admin.lists_booking",compact("username","list_booking","list_room"));
    }
    ...
    public function createBooking(Request $request){
        ...
        $list_room=DB::table('rooms')                    
                    ...
                    ->where('rooms.status', 0)
                    ->get();
        return view('admin.form',compact("username","list_room"));
    }
    ...
    public function saveBooking(Request $request){
        ...
        $exist_end_time  = room_booking::where('date',$date_booking)
            ...
            if ((!is_null($exist)) AND ($exist != $request->booking_id)){
            \Session::flash('info','There is already a booking in that start time !');    		
            return redirect()->back()->withInput()->withErrors('error');
            
        }
        if($exist_start_time != $request->booking_id){
            if (!is_null($exist_start_time)) {
                \Session::flash('info','There is already a booking between that start and  end time !');    		
                 return redirect()->back()->withInput()->withErrors('error');
            }
        }
    ...
    public function editBooking(Request $request,$id){
        ...
        $list_booking=DB::table('room_bookings')   
                        ...
                        ->select('room_bookings.*','rooms.name AS room_name','rooms.id AS room_id', 'floors.name AS floor_name', 'buildings.name AS building_name')
                        ->where('room_bookings.status', 0)
                        ->where('room_bookings.id',$id)->first(); 
        
        return view('admin.form',compact("username","list_booking","list_room"));                      
    }
    ...
    public function updateBooking(Request $request,$id){
        ...
        $room_bookings=room_booking::find($id);         
        $room_bookings->booking_by=$request->booking_by;
        $room_bookings->room_id=$request->room_name;
        $room_bookings->date=date("Y-m-d", strtotime($request->meeting_date));
        $room_bookings->start_time=date("H:i:s", strtotime($request->start_time));
        $room_bookings->end_time=date("H:i:s", strtotime($request->end_time));
        $room_bookings->purpose=$request->purpose;

        $today=date("Y-m-d");
        $date_booking=date("Y-m-d", strtotime($room_bookings->date));
        $start_time=date("H:i:s", strtotime($room_bookings->start_time));
        $end_time=date("H:i:s", strtotime($room_bookings->end_time));
        ...
        $exist=room_booking::where('date',$date_booking)    
               ...
               ->where('room_id',$request->room_name)
               ->where('start_time','<=',$start_time)
               ->where('end_time','>',$start_time)->select('id')->first();   
        ...
        $exist_start_time  = room_booking::where('date',$date_booking)
                ...
                ->where('room_id',$request->room_name)
                ->where('start_time','<',$end_time)
                ->where('start_time','>', $start_time)->select('id')->first();
        ...
        $exist_end_time  = room_booking::where('date',$date_booking)
                ...
                ->where('room_id',$request->room_name)
                ->where('start_time','<', $end_time)
                ->where('end_time','>', $end_time)->select('id')->first();
    ...
    public function deleteBooking(Request $request){
        ...
        $room_bookings=room_booking::find($request->id);
        $room_bookings->status=1;       
        if($room_bookings->save()){
